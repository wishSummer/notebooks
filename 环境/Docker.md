...
### 查看 Registry 的信息：curl -X GET http\://\<registry\_host>:\<registry\_port>/v2/

### 查看镜像列表：curl -X GET http\://\<registry\_host>:\<registry\_port>/v2/\_catalog

### 查看镜像标签：

```shell
curl -X GET http\://\<registry\_host>:\<registry\_port>/v2/\<image\_name>/tags/list
```
...
*   docker-compose.yml 文件配置 和swarm 个别配置不兼容：ip无法指定；指定节点 node.role = manager/worker,node.hostname = \[hostname]
*   docker 集群部署容器到子节点，如mysql，通过主节点Ip\:port 依旧可以连接到数据库，docker Swarm Bug点：同一个port映射 在主节点启用后，子节点部署依旧可以使用，但是访问时为子节点mysql（子节点为后部署，但先部署子节点mysql，主节点会因为port被子节点占用而无法启动容器）
